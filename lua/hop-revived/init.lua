...
local function create_hint_state(opts)
	local window = require("hop-revived.window")
...
local function set_unmatched_lines(buf_handle, hl_ns, top_line, bottom_line, cursor_pos, direction, current_line_only)
	local hint = require("hop-revived.hint")
	local prio = require("hop-revived.priority")
...
function M.get_input_pattern(prompt, maxchar, opts)
	local hint = require("hop-revived.hint")
	local jump_target = require("hop-revived.jump_target")
...
function M.setup(opts)
	...
	M.opts = setmetatable(opts or {}, { __index = require("hop-revived.defaults") })
	M.initialized = true

	-- Insert the highlights and register the autocommand if asked to.
	local highlight = require("hop-revived.highlight")
